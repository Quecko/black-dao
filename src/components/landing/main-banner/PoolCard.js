...
&:hover{
  ...
  /* filter: drop-shadow(2px 4px 6px ${props => props.colorData?.accentColor}); */
}
:first-child h4{
...
const PoolCard = ({ pool, colorData, index, stakedOnly, setLoad, settextMessage }) => {
  // function PoolCard({ pool, colorData, index, stakedOnly }) {
  ...
  const StakingP = async (isStakingTokenBnb, isRewardTokenBnb, stakingTokenAddress, contractAddress, stakeVal2, lockperiod, id) => {
    if (stakeVal === 0) {
      ...
      if (isStakingTokenBnb == false) {
        ...
        try {
          const allowanceRes = await allowanceOfToken(stakingTokenAddress, contractAddress)
          if (Web3?.utils?.fromWei(allowanceRes, 'ether')) {
            if (stakeVal2 <= Web3?.utils?.fromWei(allowanceRes, 'ether')) {
              try {
                ...
                settextMessage('Staking...')
                console.log('staking')
                const res = await stakeThePool(isStakingTokenBnb, isRewardTokenBnb, contractAddress, stakeVal2, lockperiod)
              ...
              try {
                ...
                const res = await approveThePool(stakingTokenAddress, contractAddress, stakeVal2)
  ...
  const UnstakethePoolss = async (isStakingTokenBnb, isRewardTokenBnb, contractAddress, id) => {
    try {
      setLoad(true)
      settextMessage('Unstaking...')
      window.$(`#basicModal${id}`).modal('hide');
      window.$(`#basicModalunstake${id}`).modal('hide')
      const res = await unstakeThePool(isStakingTokenBnb, isRewardTokenBnb, contractAddress)
      nftDeposit()
      nftReward()
      lockPeriodCheck()
      setLoad(false)
  ...
  const EarlyUnstakethePoolss = async (isStakingTokenBnb, isRewardTokenBnb, contractAddress, id) => {
    try {
      toast.info(`${pool?.feeforPrematureUnstaking}% unstaking fee if withdrawn within 72h ${pool?.lockPeriod}...`)
      setLoad(true)
      settextMessage(`${pool?.feeforPrematureUnstaking}% unstaking fee if withdrawn within ${pool?.lockPeriod} Seconds...`)
      window.$(`#basicModal${id}`).modal('hide');
      window.$(`#basicModalunstake${id}`).modal('hide')
      const res = await earlyunstakeThePool(isStakingTokenBnb, isRewardTokenBnb, contractAddress)
      nftDeposit()
      nftReward()
      lockPeriodCheck()
      setLoad(false)
  ...
  const HarveststhePoolsss = async (isStakingTokenBnb, isRewardTokenBnb, contractAddress) => {
    if(account){
    try {
      setLoad(true)
      settextMessage('Harvesting...')
      const res = await harvestThePool(isStakingTokenBnb, isRewardTokenBnb, contractAddress)
      console.log('res of the ui', res)
      if (res) {
        nftDeposit()
        nftReward()
  ...
  //make a funciton for the deposit

  const nftDeposit = async () => {
    try {
      setLoad(true)
      const res = await depositThePool(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress);
  ...
  const nftReward = async () => {
    try {
      setLoad(true)
      const resReward = await rewardThePool(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress);
  ...
  const lockPeriodCheck = async () => {
    try {
      const resPeriod = await lockperiodThePool(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress);
  ...
  const calculateTime = () => {
    console.log('console of the deposit time', depositValue?.depositTime, lockPeriodValue)
    console.log('console of the deposit time', parseInt(depositValue?.depositTime) + parseInt(lockPeriodValue))
    console.log('console of the deposit time', Date.now() / 1000)
    const unstaketime = parseInt(depositValue?.depositTime) + parseInt(lockPeriodValue) - Date.now() / 1000;
    // if (unstaketime > 0) {()
    setTimeLeft(unstaketime)
    // } else {

    // }
    console.log('console of the left time is xx', parseInt(depositValue?.depositTime) + parseInt(lockPeriodValue) - Date.now() / 1000)
  }
  // setTimeout(() => {
  //   calculateTime()
  // }, 1000);
  ...
  return (
    <>
      ...
      {(stakedOnly ? depositValue?.amount > 0 : true) &&
        <div className="col-xl-4 col-lg-4 col-md-6 col-12">
          <CardDiv pool={pool} colorData={colorData} className="card_item active">
            ...
            <div className="parent">
              <div className="left">
                ...
                <div className="text textIcons">
                  ...
                  <p className="textSecondary">Stake {pool?.stakingTokenSymbol || 'Bnb'}</p>
                </div>
              </div>
              {/* <div className="right">
            ...
            <div className="inner_text">
              ...
              <div className="inner marg">
                ...
                <p>{pool?.lockPeriod?.slice(0, 10) || '0'} Days</p>
              </div>
            </div>
            <div className="input_btn">
              ...
              <input value={rewardValue ? Web3?.utils?.fromWei(rewardValue, 'ether').slice(0, 216) : 0} className="px-2" disabled type="search" placeholder="0.00" />
              {pool?.autoCompound || <button disabled={depositValue?.amount <= 0 ? true : false} onClick={() => HarveststhePoolsss(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress)} className="btn-yellow">Harvest</button>}
            </div>
            <div className="bottom_item">
              <div className="text">
                <h6>{depositValue?.amount ? Web3?.utils?.fromWei(depositValue?.amount, 'ether').slice(0, 6) : 0}</h6>
                <p className="textSecondary">{pool?.stakingTokenSymbol} Staked</p>
              </div>
              <div className="btn_unstake">
                {depositValue?.amount > 0 ?
                  /* basicModalunstake${index} onClick={() => UnstakethePoolss(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress)} */
                  <button onClick={() => window.$(`#basicModalunstake${index}`).modal('show')} className="btn-yellow">UnStake</button>
                  ...
                  <div class="modal-content">
                    <div class="modal-header">
                      <h4 class="modal-title" id="myModalLabel">
                        Stake {pool?.stakingTokenSymbol}-{pool?.rewardTokenSymbol}  ({index})
                      </h4>
                      <button
                        type="button"
                        class="close"
                        data-dismiss="modal"
                        aria-label="Close"
                        onClick={() => window.$(`#basicModal${index}`).modal('hide')}
                      >
                        <img
                    ...
                    <div class="modal-body">
                      ...
                      <div className="inner-btn">
                        <input type="number" onChange={(e) => getstakeVal(e)} value={stakeVal} placeholder="0.00" />
                        <button onClick={() => setStakeVal(pool?.isStakingTokenBnb ? balance1 : tokenBalance)} className="btn-yellow">Max</button>
                      </div>
                      <p>Available balance: <span>

                        {pool?.isStakingTokenBnb ? balance1 : tokenBalance ? pool?.isStakingTokenBnb ? balance1 : tokenBalance : 0.000}
                      </span> </p>
                      {pool?.autoCompound == true ? <p>Auto Compound is Available</p> : <p>Auto Compound is Unavailable</p>}
                      ...
                      <div className="bottom-btn">
                        ...
                        <button disabled={pool?.isStakingTokenBnb ? stakeVal <= 0 || balance1 <= 0 || stakeVal > balance1 : stakeVal <= 0 || tokenBalance <= 0 || stakeVal > tokenBalance} onClick={() => StakingP(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.stakingTokenAddress, pool?.contractAddress, stakeVal, 30, index)} className={"btn-yellow"} >{'Stake'}</button>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h4 class="modal-title" id="myModalLabel">
                        UnStake {pool?.stakingTokenSymbol}-{pool?.rewardTokenSymbol}
                      </h4>
                      <button
                        type="button"
                        class="close"
                        onClick={() => window.$(`#basicModalunstake${index}`).modal('hide')}
                        data-dismiss="modal"
                        aria-label="Close"
                      >
                        <img
                    ...
                    <div class="modal-body">
                      ...
                      <div className="bottom-btn">
                        {timeLeft > 0 ?
                          <span tabindex="0" data-toggle="tooltip" title={`Early Unstake fee is ${pool?.feeforPrematureUnstaking}`}>
                            {/* <button class="btn btn-primary" style={{PointerEvents: 'none'}} type="button" disabled>Disabled button</button> */}

                            <button onClick={() => EarlyUnstakethePoolss(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress, index)} className="btn-yellow mt-4">Early UnStake</button>
                          </span>
                          :
                          <button onClick={() => UnstakethePoolss(pool?.isStakingTokenBnb, pool?.isRewardTokenBnb, pool?.contractAddress, index)} className="btn-yellow">UnStake</button>
                        }
                      </div>
                    </div>
                  </div>
